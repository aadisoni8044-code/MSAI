"""
OpenMS Canvas & Window Renderer for 2D/3D Shapes and Worlds
"""
...
class OMSRenderer:
    """Renders OpenMS shape primitives and game containers into Tkinter windows."""

    def __init__(self, root, terminal_write=None):
        self.root = root
        self.terminal_write = terminal_write or (lambda msg: None)
        self.render_window_count = 0

    def render_2d(self, obj):
        self.render_window_count += 1
        win = tk.Toplevel(self.root) if self.root else None
        if win:
            win.title(f"OpenMS - 2D Render #{self.render_window_count}")
        ...
        self.terminal_write(f"[boody_2D] Rendered {obj!r} -> 2D viewport")
        return win
    ...
    def render_3d(self, obj):
        ...
        win = tk.Toplevel(self.root) if self.root else None
        if win:
            win.title(f"OpenMS - 3D Render #{self.render_window_count}")
            win.configure(bg="#1e1e1e")
            win.geometry("520x440")
            canvas = tk.Canvas(win, width=490, height=380, bg="#0d0d10", highlightthickness=0)
            canvas.pack(padx=12, pady=12)
            self._draw_shape_3d(canvas, obj, 245, 200)
        self.terminal_write(f"[boody_3D] Rendered {obj!r} -> 3D viewport")
        return win

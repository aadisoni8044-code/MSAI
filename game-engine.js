...
function drawPlayer(ctx) {
    ...
    // 2. Draw equipped shape vehicle
    ctx.save();
    ctx.translate(player.x, player.y);
    ctx.rotate(player.angle);

    // Back radial glow
    const glowGrad = ctx.createRadialGradient(0, 0, 2, 0, 0, player.width * 0.9);
    glowGrad.addColorStop(0, skin.glowColor || skin.color);
    ...
    if (typeof window.drawShapePath === 'function') {
        window.drawShapePath(ctx, skin.shapeType || 'arrow', player.width);
    } else {
        ctx.beginPath();
    ...
    if (skin.shapeType === 'ring') {
        ctx.beginPath();
        ctx.arc(0, 0, player.width * 0.25, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(2, 6, 6, 0.95)';
        ctx.fill();
        ctx.stroke();
    }

    // Inner detail overlay pattern
    ctx.strokeStyle = skin.trailColor || '#ffffff';
    ctx.shadowColor = skin.trailColor || '#ffffff';
    ctx.lineWidth = 1.4;
    ctx.beginPath();
    ctx.arc(0, 0, player.width * 0.2, 0, Math.PI * 2);
    ctx.stroke();

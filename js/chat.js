...
class ChatController {
      ...
      await api.sendMessage({
        ...
        onDone: async (finalText, wasAborted) => {
          ...
          if (accumulatedText) {
            ...
            await conversationManager.addMessage("assistant", accumulatedText);
          }

          // If this was the first message, generate a smart title
          if (isFirstMessage && userPrompt) {
            this.triggerTitleGeneration(userPrompt);
          }
  ...
  async triggerTitleGeneration(prompt) {
    try {
      const generatedTitle = await api.generateTitle(prompt);
      if (generatedTitle) {
        await conversationManager.updateTitle(generatedTitle);
      }
    } catch (e) {
